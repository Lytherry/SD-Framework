...
alg_names <- c("SegRNN_96_96_mse", "ModernTCN_96_96_mse", "iTransformer_96_96_mse",
...
# Extract each algorithm's MSE vector (lower is better)
...
# Since lower MSE is better, rank from low to high (best = rank 1)
...
write.csv(data.frame(AvgRank = avg_ranks), file = "mse_friedman_avg_rank.csv", row.names = TRUE)

# (2) Rank differences (lower triangle) + CD in filename
CD_str <- ifelse(CD < 0.0001, "0.0000", sprintf("%.4f", round(CD, 4)))
write.csv(rank_diff, file = paste0("mse_friedman_rank_diff_CD_", CD_str, ".csv"))

# (3) Significance direction (lower triangle)
write.csv(significance, file = paste0("mse_friedman_significance_direction_CD_", CD_str, ".csv"))
